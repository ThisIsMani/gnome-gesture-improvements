...
export class WindowManupulationGesture implements ISubExtension {
	...
	gestureUpdate(_tracker: unknown, progress: number): void {
		...
		if (progress <= 1) {
			...
			this._endAction = this._isAlreadyMaximized ? (progress >= 0.8 ? EndAction.MINIMIZE : EndAction.CLOSE) : (progress >= (1 - CLOSE_THRESHOLD) ? EndAction.CANCEL : EndAction.CLOSE);
		...
		} else if (!this._isAlreadyMaximized) {
	...
	gestureEnd(_tracker: unknown, duration: number, progress: CloseWindowGestureState) {
		switch (progress) {
			...
			case CloseWindowGestureState.PINCH_IN:
				if (this._endAction === EndAction.CANCEL)
					this._animatePreview(false, duration);
				else
	...
	private _animatePreview(gestureCompleted: boolean, duration: number, callback?: () => void) {
		if (!gestureCompleted) {
			...
			if (this._endAction === EndAction.CLOSE || this._endAction === EndAction.MINIMIZE) {
				easeActor(this._preview, {
					opacity: 255,
					scaleX: this._isAlreadyMaximized && this._endAction === EndAction.MINIMIZE ? 0.8 : 0,
					scaleY: this._isAlreadyMaximized && this._endAction === EndAction.MINIMIZE ? 0.8 : 0,
					duration,
					mode: Clutter.AnimationMode.EASE_OUT_QUAD,
					onStopped: () => {
						if (callback)
